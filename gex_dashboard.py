...
# ------------------------- Cboe live feed (shared worker) ------------------
WORKER_URL = "https://gex-data.josemurill287.workers.dev"
DATA_SOURCE = "cboe"   # set from the sidebar; "cboe" (worker) or "yahoo" (yfinance)


@st.cache_data(ttl=300, show_spinner=False)
def load_all_cboe(ticker, n_exps, endpoint, _bucket=0):
    """Same shape as load_all(), but sourced from the Cboe worker (one request)."""
    import json, urllib.request, urllib.parse
    url = f"{endpoint}?symbol={urllib.parse.quote(ticker)}"
    req = urllib.request.Request(url, headers={"Accept": "application/json"})
    with urllib.request.urlopen(req, timeout=15) as r:
        j = json.loads(r.read().decode("utf-8"))
    if j.get("error") or not j.get("rows"):
        raise RuntimeError(j.get("error") or "no rows from worker")
    spot = float(j["spot"])
    today = date.today()
    by_exp = {}
    for row in j["rows"]:
        by_exp.setdefault(row["exp"], []).append(row)
    exps = sorted(by_exp)[:n_exps]
    gcols, vcols, ccols, volcols, stats = {}, {}, {}, {}, {}
    iv_skew = None
    for i, e in enumerate(exps):
        ...
        for row in by_exp[e]:
            K = float(row["strike"])
            cOI, pOI = float(row.get("cOI", 0) or 0), float(row.get("pOI", 0) or 0)
            iv = float(row.get("iv", 0) or 0)
            net_oi = cOI - pOI
            if iv > 0.001 and (cOI > 0 or pOI > 0):
                g[K] = spot * spot * 0.01 * bs_gamma(spot, K, t, iv) * net_oi * 100
                v[K] = spot * 0.01 * bs_vanna(spot, K, t, iv) * net_oi * 100
                c[K] = spot * 0.01 * bs_charm(spot, K, t, iv) * net_oi * 100
                ivs[K] = iv
            vol[K] = float(row.get("cVol", 0) or 0) + float(row.get("pVol", 0) or 0)
            if cOI > 0: coi[K] = cOI
            if pOI > 0: poi[K] = pOI
            cvol += float(row.get("cVol", 0) or 0); pvol += float(row.get("pVol", 0) or 0)
        if g: gcols[e] = pd.Series(g)
        if v: vcols[e] = pd.Series(v)
        if c: ccols[e] = pd.Series(c)
        if vol: volcols[e] = pd.Series(vol)
        coi_sum, poi_sum = sum(coi.values()), sum(poi.values())
        atm_iv = ivs[min(ivs, key=lambda x: abs(x - spot))] if ivs else None
        stats[e] = {
            "pc_oi": (poi_sum / coi_sum) if coi_sum > 0 else None,
            "pc_vol": (pvol / cvol) if cvol > 0 else None,
            "max_pain": max_pain(coi, poi),
            "atm_iv": atm_iv,
            "dte": (datetime.strptime(e, "%Y-%m-%d").date() - today).days,
        }
        if i == 0 and ivs:
            iv_skew = pd.Series(ivs).sort_index()
    return {
        "spot": spot, "day_chg": 0.0,   # worker doesn't carry prior close
        "gex": pd.DataFrame(gcols).sort_index() if gcols else pd.DataFrame(),
        "vex": pd.DataFrame(vcols).sort_index() if vcols else pd.DataFrame(),
        "charm": pd.DataFrame(ccols).sort_index() if ccols else pd.DataFrame(),
        "vol": pd.DataFrame(volcols).sort_index() if volcols else pd.DataFrame(),
        "stats": stats, "iv_skew": iv_skew, "source": "cboe",
    }


def get_data(ticker, n_exps, bucket):
    """Dispatch to the Cboe worker or Yahoo, with graceful fallback."""
    if DATA_SOURCE == "cboe":
        try:
            return load_all_cboe(ticker, n_exps, WORKER_URL, bucket)
        except Exception:
            d = load_all(ticker, n_exps, bucket)
            if d:
                d = dict(d); d["source"] = "yahoo-fallback"
            return d
    return load_all(ticker, n_exps, bucket)
...
# ---- link to the standalone HEATSEEKER-style heatmap site (opens in a new tab) ----
# After you host index.html (Netlify/GitHub Pages), paste that URL here:
HEATMAP_URL = "https://ephemeral-crumble-1d1556.netlify.app"   # live heatmap site

_hcol1, _hcol2 = st.columns([3, 1])
with _hcol2:
    if HEATMAP_URL and "YOUR-HEATMAP-SITE" not in HEATMAP_URL:
        if hasattr(st, "link_button"):
            st.link_button("🔥 Open live Heatmap →", HEATMAP_URL, use_container_width=True)
        else:
            st.markdown(f"<a href='{HEATMAP_URL}' target='_blank' "
                        f"style='display:block;text-align:center;padding:8px;border-radius:10px;"
                        f"background:#35d49a;color:#0b0e14;font-weight:800;text-decoration:none;'>"
                        f"🔥 Open live Heatmap →</a>", unsafe_allow_html=True)
    else:
        st.caption("Set HEATMAP_URL in the code to link your heatmap here.")


with st.sidebar:
    st.header("Settings")
    src_choice = st.radio("Data source", ["Cboe live (worker)", "Yahoo (yfinance)"], index=0)
    DATA_SOURCE = "cboe" if src_choice.startswith("Cboe") else "yahoo"
...
data = get_data(ticker, n_exps, bucket)
if not data:
