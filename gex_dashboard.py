...
# Optional auto-refresh helper. If the package isn't installed, we fall back to
# a sleep+rerun loop later — so the app never crashes over a missing dependency.
try:
...
def load_grid_both(ticker, n_exps, _bucket=0):
    # _bucket changes each auto-refresh interval so the cache re-fetches fresh data
...
with st.sidebar:
    ...
    interval = st.selectbox("Every", [30, 60, 120, 300], index=1,
                            ...
                            disabled=not auto)
    st.caption("Yahoo data is ~15 min delayed and OI updates once daily, so 60s+ is plenty. "
...
bucket = 0
if auto:
    bucket = int(time.time() // interval)          # new value each interval -> fresh fetch
    if HAS_AUTOREFRESH:
        st_autorefresh(interval=interval * 1000, key="auto_refresh")
    # (if the helper isn't installed, a sleep+rerun fallback runs at the end of the script)
    st.markdown(
...
spot, gdf, vdf, day_chg = load_grid_both(ticker, n_exps, bucket)
...
# Fallback auto-refresh: only used if the streamlit-autorefresh package isn't installed.
